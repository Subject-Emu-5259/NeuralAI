...
Chapter 5 of [Web Browser Engineering](https://browser.engineering/index.html "Table of Contents").
[<](https://browser.engineering/html.html "Previous chapter")
[>](https://browser.engineering/styles.html "Next chapter")
...
* [The Layout
  Tree](https://browser.engineering/layout.html#the-layout-tree)
* [Block Layout](https://browser.engineering/layout.html#block-layout)
* [Size and
  Position](https://browser.engineering/layout.html#size-and-position)
* [Recursive
  Painting](https://browser.engineering/layout.html#recursive-painting)
* [Backgrounds](https://browser.engineering/layout.html#backgrounds)
* [Summary](https://browser.engineering/layout.html#summary)
* [Outline](https://browser.engineering/layout.html#outline)
* [Exercises](https://browser.engineering/layout.html#exercises)
...
that, this chapter switches to *tree-based layout*, where the
...
That said, requiring a `parent` and `previous`
object now makes it tricky to construct a `Layout` object in
...
Note an interesting thing about this new `layout` method:
...
`BlockLayout` as a name, because we ultimately want it to
represent a block of text, like a paragraph or a heading:
...
don’t match! Chrome calls it a [layout
tree](https://developers.google.com/web/updates/2018/09/inside-browser-part3), Safari a [render
tree](https://webkit.org/blog/114/webcore-rendering-i-the-basics/), and Firefox a [frame
tree](https://wiki.mozilla.org/Gecko:Key_Gecko_Structures_And_Invariants).

Block Layout
============

So far, we’ve focused on text layout—and text is laid out
horizontally in lines.In
European languages, at least! But web pages are really
...
involves expanding on the layout tree we’ve already built.
...
there will also be new, intermediate `BlockLayout`s with
...
play with how block layout leads to different visual output.)

![Figure 1: An example of an HTML tree and the corresponding layout tree.](https://browser.engineering/im/layout-tree-2.gif)

Figure 1: An example of an HTML tree and
...
I’ve called this method `layout_intermediate`, but only so
...
HTML node contains: *inline* text and text-related tags like
`<b>`, or *blocks* like `<p>`
and `<h1>`. Let’s add a `layout_mode` method
...
Here, the list of `BLOCK_ELEMENTS` is basically what you
expect, a list of all the tags that describe blocks and containers:Taken from the [HTML living
standard](https://html.spec.whatwg.org/multipage/#toc-semantics).
...
implicit tags in [Chapter 4](https://browser.engineering/html.html), we need a repair
...
block boxes](https://developer.mozilla.org/en-US/docs/Web/CSS/Visual_formatting_model#anonymous_boxes)” and is more complex than what’s described here; see
Exercise 5-5.
...
Finally, since `BlockLayout`s can now have children, the
...
complex layout trees! Now we need each of these `BlockLayout`
objects to have a size and position somewhere on the page.

In CSS, the layout mode is set by the [`display`
property](https://developer.mozilla.org/en-US/docs/Web/CSS/display). The oldest CSS layout modes, like `inline` and
`block`, are set on the children instead of the parent, which
leads to hiccups like [anonymous
block boxes](https://developer.mozilla.org/en-US/docs/Web/CSS/Visual_formatting_model#anonymous_boxes). Newer properties like `inline-block`,
...
In the [previous chapter](https://browser.engineering/html.html), the
...
Do the same for `DocumentLayout`. Now we need to update
...
recall that `BlockLayout`s represent blocks of text like
...
the [next chapter](https://browser.engineering/styles.html), we’ll add support for
...
However, a `BlockLayout` that contains text doesn’t have
...
* When `layout` is called, it first computes the
...
* Finally, `layout` computes the `height` field,
  reading from the child layout objects.

You can see these steps in action in this widget:

![Figure 2: A flowchart showing how widths are computed top-down, from parent to child, while heights are computed bottom-up, from child to parent.](https://browser.engineering/im/layout-order.png)

Figure 2: A flowchart showing how widths
...
back to this issue of dependencies [in
Chapter 16](https://browser.engineering/invalidation.html), where it will become even more important.
...
Formally, computations on a tree like this can be described by an [attribute
grammar](https://en.wikipedia.org/wiki/Attribute_grammar). Attribute grammar engines analyze dependencies between
...
Now the browser can use `paint_tree` to collect its own
...
Layout trees are common [in
graphical user interface (GUI) frameworks](https://book.huihoo.com/debian-gnu-linux-desktop-survival-guide/Widget_Tree.html), but there are other ways
to structure layout, such as constraint-based layout. TeX’s [boxes
and glue](https://www.overleaf.com/learn/latex/Articles/Boxes_and_Glue%3A_A_Brief%2C_but_Visual%2C_Introduction_Using_LuaTeX) and iOS’s [auto-layout](https://developer.apple.com/library/archive/documentation/UserExperience/Conceptual/AutolayoutPG/index.html)
are two examples of this alternative paradigm.

Backgrounds
===========

Browsers use the layout tree a lot,For example, in [Chapter 7](https://browser.engineering/chrome.html), we’ll
use the size and position of each link to figure out which one the user
clicked on. and one simple and visually compelling use
case is drawing backgrounds.

Backgrounds are rectangles, so our first task is putting rectangles
...
*commands*, of which there is currently only one type. We now
want two types of commands:
...
Now `BlockLayout` must add `DrawText` objects
...
But it can also add a `DrawRect` command to draw a
...
With the display list filled out, we need to draw each graphics
command. Let’s add an `execute` method for this. On
...
Note that `execute` takes the scroll amount as a
...
By default, `create_rectangle` draws a one-pixel black
...
The browser’s `draw` method now just uses `top`
...
Try your browser on a page—maybe [this chapter’s](https://browser.engineering/layout.html)—with
code snippets on it. You should see each code snippet set off with a
gray background.

Here’s one more cute benefit of tree-based layout: we now record the
...
Note the `2*VSTEP`, to account for a `VSTEP` of
whitespace at the top and bottom of the page. With layout the [browser.engineering homepage](https://browser.engineering/) now
looks a bit better—see Figure 3.

So those are the basics of tree-based layout! In fact, as we’ll see
...
some styling capabilities to our browser.

![Figure 3: https://browser.engineering/ viewed in this chapter’s version of the browser.](https://browser.engineering/examples/example5-browserengineering-screenshot.png)

Figure 3: https://browser.engineering/
viewed in this chapter’s version of the browser.

The draft CSS [Painting
API](https://developer.mozilla.org/en-US/docs/Web/API/CSS_Painting_API/Guide) allows pages to extend the display list with new types of
commands, implemented in JavaScript. This makes it possible to use CSS
for styling with visually complex styling provided by a library.
...
* each node in the tree has one of two different *layout
  modes*;
...
browser’s styling capabilities. We’ll work on that in the [next chapter](https://browser.engineering/styles.html).

[Close](https://browser.engineering/layout.html)
...
5-5 *Anonymous block boxes*. Sometimes, an element has a mix
of text-like and container-like children. For example, in this HTML,

```
<div><i>Hello, </i><b>world!</b><p>So it began...</p></div>
```

the `<div>` element has three children: the
`<i>`, `<b>`, and
...
`display: run-in` property [is poor](https://caniuse.com/run-in), this book actually
