...
Welcome back to my series on building your own toy browser engine. If you’re
just tuning in, you can find the previous episodes here:
...
values](http://www.w3.org/TR/CSS2/cascade.html), or what I call the [style](https://github.com/mbrubeck/robinson/blob/master/src/style.rs) module.
...
> **What’s with all the `'a` stuff?** Those are [lifetimes](http://doc.rust-lang.org/book/ownership.html),
...
code](https://wiki.mozilla.org/Gecko:Key_Gecko_Structures_And_Invariants) takes a DOM tree and produces a *frame tree*, which is
then used to build a *view tree*. Blink and WebKit transform the DOM tree
into a [*render tree*](http://dev.chromium.org/developers/design-documents/gpu-accelerated-compositing-in-chrome). Later stages in all these engines produce
...
could exclude elements whose [`display`](https://developer.mozilla.org/en-US/docs/Web/CSS/display) property is set to
...
The first step in building the style tree is [selector matching](http://www.w3.org/TR/CSS2/selector.html#pattern-matching).
This will be very easy, since my [CSS parser](https://limpet.net/mbrubeck/2014/08/13/toy-layout-engine-3-css.html) supports only simple
...
type](https://github.com/mbrubeck/robinson/blob/master/src/dom.rs). The `class` attribute can contain multiple class names
...
selector component, and return `false` if the element doesn’t have a matching
...
> **Rust note:** This function uses the [`any`](http://doc.rust-lang.org/core/iter/trait.Iterator.html#method.any) method, which
> returns `true` if an iterator contains an element that passes the provided
> test. This is the same as the [`any`](https://docs.python.org/3/library/functions.html#any) function in Python ([or
> Haskell](http://hackage.haskell.org/package/base-4.7.0.1/docs/Prelude.html#v:any)), or the [`some`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/some) method in JavaScript.
...
styles](http://www.w3.org/TR/CSS2/sample.html) via *user agent style sheets*. And they may allow users
...
The [cascade](http://www.w3.org/TR/CSS2/cascade.html#cascade) defines which of these three “origins” takes
...
In addition to the “specified values” mentioned above, CSS defines [*initial*,
*computed*, *used*, and *actual* values](http://www.w3.org/TR/CSS2/cascade.html#value-stages).
...
other styles? The answer is [inheritance](http://www.w3.org/TR/CSS2/cascade.html#inheritance).
...
`parse_declarations` from the [CSS parser](https://limpet.net/mbrubeck/2014/08/13/toy-layout-engine-3-css.html). Apply the resulting
...
[Part 5](https://limpet.net/mbrubeck/2014/09/08/toy-layout-engine-5-boxes.html) will introduce the layout module. I haven’t
...
implementation](http://www.wambo.at:8080/GyrosOfWar/browserino/tree/master) and Pohl Longsine’s [Swift version](http://www.screaming.org/blog/categories/crow/).

[Subscribe](https://limpet.net/mbrubeck/atom.xml)