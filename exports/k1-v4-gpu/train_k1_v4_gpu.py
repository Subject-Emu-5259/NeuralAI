...
def resolve_base(base_arg):
    """Allow local path or HuggingFace model id. Falls back to canonical K1 base."""
    if base_arg:
        if os.path.exists(base_arg):
            return base_arg
        return base_arg
    local = "models/k1/base"
...
def train(args):
    ...
    print(f"[{datetime.now().strftime('%H:%M:%S')}] Training {args.run_name}", flush=True)
    print(f"  Device: {device}, dtype: {dtype}", flush=True)
    print(f"  Base: {args.base}", flush=True)
    print(f"  Data: {args.data}", flush=True)
    print(f"  Steps: {args.max_steps}, Batch: {args.batch_size}, GradAccum: {args.grad_accum}", flush=True)
    ...
    print(f"  Model loaded in {time.time() - t0:.1f}s | params={sum(p.numel() for p in model.parameters())/1e6:.1f}M", flush=True)
    ...
    print(f"  LoRA trainable: {trainable/1e6:.2f}M / {total/1e6:.1f}M ({100*trainable/total:.2f}%)", flush=True)
    ...
    while step < args.max_steps:
        for batch in loader:
            ...
            if step % args.save_every == 0 and step > 0:
                ...
                print(f"  -> Saved checkpoint-{step} | avg_loss={avg_loss:.4f}", flush=True)
                if avg_loss < best_loss:
                    best_loss = avg_loss
                    best_dir = os.path.join(output_dir, "best")
                    model.save_pretrained(best_dir)
                    tokenizer.save_pretrained(best_dir)
                    print(f"  -> New best! loss={best_loss:.4f} -> {best_dir}", flush=True)
    ...
    elapsed = time.time() - start_time
    print(f"\n{'='*50}", flush=True)
    print(f"Training complete: {step} steps in {elapsed/60:.1f}m", flush=True)
    print(f"Final loss: {avg_loss:.4f} | Best loss: {best_loss:.4f}", flush=True)
