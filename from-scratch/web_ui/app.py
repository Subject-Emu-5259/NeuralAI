...
try:
    ...
    from neuralai_engine import neuralai_chat, local_model, neuralai_tool_call, _truncate_messages, MAX_CONTEXT_TOKENS
    HAS_ROUTER = True
...
def query_uplink(user_msg: str, conversation_history: list[dict]) -> str:
    ...
    # Ensure the system prompt leads the conversation context sent to uplink.
    system_msg = {"role": "system", "content": get_system_prompt()}
    past = conversation_history[-6:] if conversation_history else []
    if not any(m.get("role") == "system" for m in past):
        past = [system_msg] + past
    payload = {
        "task": user_msg,
        "context": {"conversation": past},
    }
    try:
...
def answer_with_model_stream(messages: list[dict], user_content: str, doc_context: str, max_new_tokens: int, temperature: float, founder_mode=False):
    ...
    try:
        ...
        if _truncate_messages is not None:
            messages = _truncate_messages(messages, max_tokens=MAX_CONTEXT_TOKENS)
