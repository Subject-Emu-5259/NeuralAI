...
def _estimate_tokens(text: str) -> int:
    """Rough token estimator used for context trimming."""
    return int(len(text.split()) * 1.3)


def _truncate_messages(messages, max_tokens=MAX_CONTEXT_TOKENS):
    """Keep the system message and the most recent user message, trimming oldest
    non-system turns until the estimated token count is below max_tokens."""
    system = [m for m in messages if m.get('role') == 'system']
    convo = [m for m in messages if m.get('role') != 'system']

    # Anchor the most recent user message (and any final assistant reply).
    if convo and convo[-1].get('role') == 'user':
        anchor = convo[-1:]
        trunk = convo[:-1]
    elif len(convo) >= 2 and convo[-2].get('role') == 'user' and convo[-1].get('role') == 'assistant':
        anchor = convo[-2:]
        trunk = convo[:-2]
    else:
        anchor = []
    ...
    total = sum(_estimate_tokens(m.get('content', '')) for m in system + trunk + anchor)
    while trunk and total > max_tokens:
        dropped = trunk.pop(0)
        total -= _estimate_tokens(dropped.get('content', ''))

    return system + trunk + anchor
