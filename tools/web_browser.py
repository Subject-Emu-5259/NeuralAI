...
_BROWSER_MODE = os.getenv("NEURALAI_BROWSER_MODE", "local").lower()
_REMOTE_CDP_URL = os.getenv("NEURALAI_REMOTE_BROWSER_CDP_URL", "").strip()
_REMOTE_WSE_URL = os.getenv("NEURALAI_REMOTE_BROWSER_WSE_URL", "").strip()


def _remote_enabled() -> bool:
    return _BROWSER_MODE == "remote" and bool(_REMOTE_CDP_URL or _REMOTE_WSE_URL)
...
class BrowserManager:
    ...
    def _loop(self):
        ...
        try:
            ...
            if _REMOTE_CDP_URL:
                browser = p.chromium.connect_over_cdp(_REMOTE_CDP_URL)
            elif _REMOTE_WSE_URL:
                browser = p.chromium.connect(_REMOTE_WSE_URL)
            else:
